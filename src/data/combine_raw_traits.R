...
# sample identifiers
samp <- read_excel(here("data", "raw", "sev_plant_traits_collected_samples_2017.xlsx"), 
                   sheet=1) %>%
...
# leaf area
lf_area <- read_excel(here("data", "raw", "sev_leaf_area_2017-18.xlsx"), sheet=1) %>% 
  group_by(code) %>%
...
# leaf mass
lf_mass <- read_excel(here("data", "raw", "sev_leaf_masses_2017-18.xlsx"), sheet=1) %>%
  rename(lf_wet_mass = wet, 
         lf_dry_mass = dry,
         lf_petiole_ind = petiole)

# leaf isotopes
lf_iso <- read_excel(here("data", "raw", "sev_plant_isotopes_2017-18.xlsx"), sheet=1) %>%
  select(code = id, mass:cn)

# stem and root data
st_rt <- read_excel(here("data", "raw", "sev_stem_root_data_2017-18.xlsx"), sheet=1) %>%
  mutate_at(vars(code), funs(substr(., 2, nchar(.)))) # remove leading "p"

# fine root shapes
fr_shp <- read_csv(here("data", "processed", "fine_root_shapes_2017-18.csv")) %>%
  mutate_at(vars(code), funs(as.character(.)))

# fine root mass
fr_mass <- read_excel(here("data", "raw", "fine_root_mass.xlsx")) %>%
  mutate_at(vars(code), funs(as.character(.)))

# seed mass
seed_mass <- read_excel(here("data", "raw", "sev_seed_masses.xlsx")) %>% 
...
indv_data <- samp %>%
  ...
  write_csv(file = here("data", "processed", "indv_samp_traits_2017-18.csv"))
